  ...
  Alpine.store('exercises', {
    ...
    get filteredItems() {
      const filtered = this.items.filter(ex => {
        const searchClean = removeVietnameseTones(this.search.toLowerCase());
        const matchesSearch = !this.search || 
          removeVietnameseTones(ex.name_eng.toLowerCase()).includes(searchClean) ||
          (ex.name_vie && removeVietnameseTones(ex.name_vie.toLowerCase()).includes(searchClean)) ||
          (ex.primary_muscle && removeVietnameseTones(ex.primary_muscle.toLowerCase()).includes(searchClean)) ||
          (ex.secondary_muscle && ex.secondary_muscle.some(m => removeVietnameseTones(m.toLowerCase()).includes(searchClean)));
